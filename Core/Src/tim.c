...
/* USER CODE BEGIN 0 */
#define ICP_BUF_SIZE 4 	//采样通道数input capture sample channel counter val

/*
 * //the data array of mutilchannel sample to transport DMA
 * //全局变量，ICP_val_buf[0][]存放上升沿TC，ICP_val_buf[1][]存放下降沿TC
 */
static uint16_t ICP_val_buf[2][ICP_BUF_SIZE] = {0};
static uint32_t PulseBuffer[10];

uint16_t lenPulse = 0;
uint16_t lenPeriod = 0;
/* USER CODE END 0 */
...
void MX_TIM1_Init(void)
{
  ...
  HAL_TIM_PWM_Init(&htim1);
	/* å®šæ—¶å™¨ï¿½?ï¿½é�?1è¾“å�?�ºPWM */
	HAL_TIM_PWM_Start(&htim1,TIM_CHANNEL_1);
	/* å®šæ—¶å™¨ï¿½?ï¿½é�?1äº’è¡¥è¾�?�å�?�ºPWM */
...
//void HAL_TIM_PWM_PulseFinishedCallback(TIM_HandleTypeDef *htim)//å‡½æ�?��?2
//{
//	if(htim==&htim8)
//	{
//		HAL_TIM_PWM_Stop_DMA(&htim8, TIM_CHANNEL_2);
...
/****************************************************************************
 *
 * å‡½æ�?�°åï¼�?          pulse_StructInit
 * è¾“å�?�¥ï�????????????????            ï¿???????????????
 * è¾“å�?�ºï�????????????????            ï¿???????????????
 * è¿”å�?�žï�????????????????:           ï¿???????????????
 * æè¿°ï¿???????????????            ç»“æž�?�ä½�?�åˆå§�?�åŒ�??
 ***************************************************************************/
void pulse_StructInit ()
{
	uint32_t i = 0;
	for(i = 0; i < 10; i++)
	{
